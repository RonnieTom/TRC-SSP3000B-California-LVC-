...
local extra_keys = {
	...
	{key = 40, type = 'right'},  -- ]
	{key = 74, type = 'aux'},    -- H
	{key = 344, type = 'park'}   -- F11
}
local extra_state = {left=false, right=false, aux=false, park=false}
	...
	while true do
		...
		if GetPedInVehicleSeat(veh, -1) == playerped then
			...
			if GetVehicleClass(veh) == 18 then
					...
					if not IsPauseMenuActive() then
						if not key_lock and not radio_wheel_active then
							...
							elseif IsDisabledControlJustReleased(0, 172) and not IsMenuOpen() then
								if state_pwrcall[veh] == 0 then
									if state_lights then
										...
										count_bcast_timer = delay_bcast_timer
										SendNUIMessage({ _type = 'hud:showPA', show = true })
									end
								else
									AUDIO:Play('Downgrade', AUDIO.downgrade_volume)
									if state_lxsiren[veh] == 0 then
									...
									SetPowercallStateForVeh(veh, 0)
									SendNUIMessage({ _type = 'hud:showPA', show = false })
								end
								AUDIO:ResetActivityTimer()
								count_bcast_timer = delay_bcast_timer
							end
							...
							if IsDisabledControlJustPressed(0, 80) then
								SendNUIMessage({ _type = 'hud:showManon', show = true })
							end
							if IsDisabledControlJustReleased(0, 80) then
								SendNUIMessage({ _type = 'hud:showManon', show = false })
							end
							if IsDisabledControlJustPressed(0, 80) then
								SendNUIMessage({ _type = 'hud:showManon', show = true })
							end
				...
				-- 为所有非 VC_EMERGENCY（VEHICLES.META）的车辆禁用警报器音频
				TogMuteDfltSrnForVeh(veh, true)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if player_is_emerg_driver and veh ~= nil then
			...
			local config = vehicle_extras_config and vehicle_extras_config[string.lower(veh_name)]
			if config then
				for i, v in ipairs(extra_keys) do
					local extra_num = config['extra'..i]
					if extra_num then
						if IsControlJustPressed(0, v.key) then
							extra_state[v.type] = not extra_state[v.type]
							SetVehicleExtra(veh, extra_num, not extra_state[v.type])
							SendNUIMessage({ _type = 'hud:setExtraHud', extra = v.type, show = extra_state[v.type] })
							SendNUIMessage({ _type = 'audio', file = 'ST300/Downgrade', volume = 0.7 })
						end
					end
				end
			end
...
AddEventHandler('lvc:onVehicleChange', function()
	...
	if player_is_emerg_driver and veh ~= nil then
		...
		if config and config.default then
			for _, extra_num in ipairs(config.default) do
				SetVehicleExtra(veh, extra_num, false)
			end
		end
		-- 重置HUD显示
		for k in pairs(extra_state) do
			extra_state[k] = false
			SendNUIMessage({ _type = 'hud:setExtraHud', extra = k, show = false })
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	while true do
		Citizen.Wait(500)
		if player_is_emerg_driver and veh ~= nil then
			local engine_running = GetIsVehicleEngineRunning(veh)
			if engine_running ~= last_engine_state then
				last_engine_state = engine_running
				SendNUIMessage({
					_type = 'hud:setEngineState',
